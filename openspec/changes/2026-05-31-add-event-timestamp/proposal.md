...
- **Data consistency**: The column is computed at insert time. For existing rows, a backfill migration would be needed if we want historical timestamps. Backfill SQL: `UPDATE device_wlan_connections_fact f JOIN date_dim d ON f.date_key=d.date_key JOIN time_dim t ON f.time_key=t.time_key SET f.event_timestamp = CAST(CONCAT(DATE_FORMAT(d.date,'%Y%m%d'), DATE_FORMAT(SEC_TO_TIME(t.time),'%H%i%s')) AS UNSIGNED)`
